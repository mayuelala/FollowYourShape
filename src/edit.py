...
from flux.sampling import denoise, get_schedule, prepare, unpack, denoise_with_importance, build_inject_list
...
def main(
    ...
    while opts is not None:
        ...
        inject_list = build_inject_list(num_inference_steps=len(timesteps), inject_step=info['inject_step'], tail_pad=1, front_pad=3) 
        ...
        x, _ = denoise_with_importance(model, **inp_target, timesteps=timesteps, guidance=guidance, inverse=False, info=info, width=opts.width, height=opts.height, inject_list=inject_list, tail_pad=1, front_pad=3,
                                       controlnet=controlnet, control_patch=control_patch, controlnet_scale=controlnet_scale, controlnet_mode=control_mode, guidance_start=guidance_start, guidance_end=guidance_end)       
